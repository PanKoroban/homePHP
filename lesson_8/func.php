...
// 1 - В корзине
// 2 - Оплачен
// 3 - Выполнен (Доставлен)
function Order($link, $status){
$queryOrder = "select b.goods_count, b.id, c.name, c.price from basket b 
inner join catalog c on b.goods_id = c.id
where user_login='{$_SESSION['login']}' and basket_status = $status";
...
$resOrder = mysqli_query($link, $queryOrder);
    ...
    while($row = mysqli_fetch_assoc($resOrder)):?>
    <div class="basket_card">
    <p><?= $row['name'] ;?> <?= $row['goods_count'];?> шт.</p>
    <p>Сумма: <?= (int)$row['price']*(int)$row['goods_count']; ?> </p> 
        <?php $sum=$sum+(int)$row['price']*(int)$row['goods_count']; ?>
    </div>
    <?php endwhile; 
